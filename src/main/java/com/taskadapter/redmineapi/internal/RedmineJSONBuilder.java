...
public class RedmineJSONBuilder {
    ...
    */
    ...
    public static void writeProject(Project project, final JSONWriter writer)
        ...
        writeModules(writer, project.getModules());
        writeTrackers(writer, project.getTrackers());
...
//		addIfSet(writer, "name", storage, User.FULL_NAME);
    ...
    public static void writeModules(JSONWriter writer, Collection<String> modules)
            throws JSONException {
            if (modules == null || modules.isEmpty()) {
                return;
            }

            writer.key("enabled_module_names");
            writer.array();
    ...
    public static void writeTrackers(JSONWriter writer, Collection<Tracker> trackers)
            throws JSONException {
            if (trackers == null || trackers.isEmpty()) {
                return;
            }

            writer.key("tracker_ids");
            writer.array();
            for (Tracker tracker : trackers) {
                if (tracker.getId() != null) {
                    writer.value(tracker.getId().longValue());
                }
            }
            writer.endArray();
    }
